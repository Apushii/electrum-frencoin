...
APPIMAGE="$DISTDIR/electrum-frencoin-$VERSION-x86_64.AppImage"
...
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-frencoin-pypi.txt"
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-frencoin-git.txt"
...
cp "$PROJECT_ROOT/electrum-frencoin.desktop" "$APPDIR/electrum-frencoin.desktop"
