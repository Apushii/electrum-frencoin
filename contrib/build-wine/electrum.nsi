...
;Variables
  ...
  !define PRODUCT_NAME "Electrum Frencoin"
  !define PRODUCT_WEB_SITE "https://github.com/Apushii/electrum-frencoin"
  !define PRODUCT_PUBLISHER "The Frencoin Community"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-frencoin-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-frencoin-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-frencoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-frencoin-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-frencoin-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-frencoin-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: and lightning: URIs to Electrum
  WriteRegStr HKCU "Software\Classes\fren" "" "URL:fren Protocol"
  WriteRegStr HKCU "Software\Classes\fren" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\fren" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\fren\shell\open\command" "" "$\"$INSTDIR\electrum-frencoin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  ;WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-frencoin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
