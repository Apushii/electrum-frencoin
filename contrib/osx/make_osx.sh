...
PACKAGE=Electrum-Frencoin
GIT_REPO=https://github.com/Apushii/electrum-frencoin
...
info "Installing dependencies sepcific to frencoin..."
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-frencoin-pypi.txt ||
    fail "Could not install dependencies from pypi"
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-frencoin-git.txt ||
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-frencoin-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-frencoin-${VERSION}.dmg"
